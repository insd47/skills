...
Codex App 작업을 Claude Code의 native MCP tools로 위임하고 이어서 조율하는 Claude 전용 플러그인이다. persistent task의 맥락 위에 쌓이는 구현과 후속 작업을 `ask`로 전달하고, 같은 turn을 조회하거나 steer·interrupt한다. MCP server instructions가 도구 선택과 조율 규칙을 제공하며, 플러그인 자체는 별도 Skill이나 Bash 호출을 로드하지 않는다.

## 설치

marketplace를 등록하고 플러그인을 설치한다. MCP server만 현재 세션에 반영하려면 `/reload-plugins`를 사용할 수 있지만, background monitor까지 시작하려면 Claude Code 세션을 새로 열어야 한다.

Apple Silicon Mac과 Claude Code 2.1.121 이상이 필요하다. 플러그인은 배포된 단일 실행 파일을 사용하며 Node.js를 요구하지 않는다. Claude Code 최소 버전은 background monitor와 MCP `alwaysLoad`를 모두 사용할 수 있는 기준이다.
...
업데이트는 marketplace snapshot을 갱신한 뒤 플러그인을 최신 버전으로 올리는 방식으로 적용한다. 업데이트된 MCP server와 monitor를 함께 사용하려면 Claude Code 세션을 다시 시작한다.
...
`ask` MCP tool은 Codex가 발급한 `threadId`와 `turnId`를 시작 직후 반환한다. plugin 고유 job ID나 영속 job registry는 만들지 않는다. MCP server가 persistent turn을 같은 프로세스의 Tokio task로 소유하고, 완료 시점에만 `${CLAUDE_PLUGIN_DATA}` 아래에 작은 outbox message를 남긴다. plugin monitor가 이를 Claude notification으로 전달한 뒤 삭제하므로 별도 polling이 필요 없다.

`ask`를 다시 호출하면 같은 persistent 작업의 실행 중인 turn을 자동으로 steer한다. `status`, `steer`, `interrupt`는 앞서 받은 Codex `threadId`와 `turnId`를 그대로 사용한다. 별도 상태 목록은 제공하지 않으며 `status`는 해당 turn이 현재 MCP server 세션의 메모리 레지스트리에 있는지만 확인한다. `interrupt`는 Codex turn만 정상 중단하고 task는 보존한다. 서버 재시작 전후로 제어 상태를 이어 붙이지 않는다.
...
MCP server와 monitor는 `src/`의 같은 Rust crate에서 빌드하며 `server`, `monitor` 두 실행 mode만 제공한다. stdio MCP에는 공식 `rmcp` SDK를 사용하고 wire type은 Serde로 정의한다.
